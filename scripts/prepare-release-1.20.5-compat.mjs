...
await update('scripts/apply-release-1.20.7.mjs', (source) => source
  .replace(
    ...
    "if (!['1.20.7', '1.20.8', '1.20.9', '1.20.10'].includes(version)) throw new Error(`1.20.7 materializer requires a compatible 1.20.7+ chain, got ${version}.`);",
  ));
...
await update('scripts/test-release-1.20.5.mjs', (source) => source.replace(
  ...
  "if (!['1.20.5', '1.20.6', '1.20.7', '1.20.8', '1.20.9', '1.20.10'].includes(pkg.version)) throw new Error(`Unexpected package version: ${pkg.version}`);",
));
await update('scripts/test-release-1.20.6.mjs', (source) => source.replace(
  /if \(!\[[^\n]+\]\.includes\(pkg\.version\)\) throw new Error\(`Unexpected package version: \$\{pkg\.version\}`\);/,
  "if (!['1.20.6', '1.20.7', '1.20.8', '1.20.9', '1.20.10'].includes(pkg.version)) throw new Error(`Unexpected package version: ${pkg.version}`);",
));
await update('scripts/test-release-1.20.7.mjs', (source) => {
  let next = source.replace(
    /if \((?:pkg\.version !== '1\.20\.7'|!\[[^\n]+\]\.includes\(pkg\.version\))\) throw new Error\(`Unexpected package version: \$\{pkg\.version\}`\);/,
    "if (!['1.20.7', '1.20.8', '1.20.9', '1.20.10'].includes(pkg.version)) throw new Error(`Unexpected package version: ${pkg.version}`);",
  );
  if (!next.includes("await import('./test-release-1.20.9.mjs')")) {
    next = `${next.trimEnd()}\n\nif (['1.20.9', '1.20.10'].includes(pkg.version)) await import('./test-release-1.20.9.mjs');\n`;
  } else {
    next = next.replace("if (pkg.version === '1.20.9') await import('./test-release-1.20.9.mjs');", "if (['1.20.9', '1.20.10'].includes(pkg.version)) await import('./test-release-1.20.9.mjs');");
  }
  return next;
});
