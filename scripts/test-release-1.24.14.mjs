...
need(pkg.version === '1.24.14', `Expected 1.24.14, got ${pkg.version}`);
...
need(pkg.scripts?.['prepare:release'] === 'node scripts/prepare-release-1.24.14.mjs', '1.24.14 release orchestrator is not active.');

need(index.includes('data-app-version="1.24.14"'), 'Web app version was not bumped to 1.24.14.');
need(server.includes("const APP_VERSION = '1.24.14';"), 'Server version was not bumped to 1.24.14.');
need(serviceWorker.includes('flyxora-v1.24.14-retired-news-source-cleanup'), '1.24.14 service-worker cache key is missing.');
...
console.log('FLYXORA 1.24.14 retired News source cleanup regression passed.');
