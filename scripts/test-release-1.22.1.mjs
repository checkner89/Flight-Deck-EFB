...
const [html, css, uxCss, ux, server, sw, runtime] = await Promise.all([
  fs.readFile('public/index.html', 'utf8'),
  fs.readFile('public/release-1.22.1.css', 'utf8'),
  fs.readFile('public/release-1.22.1-ux.css', 'utf8'),
  fs.readFile('public/release-1.22.1-ux.js', 'utf8'),
...
need(html, 'release-1.22.1-ux.css?v=1.22.1', '1.22.1 UX stylesheet not wired.');
need(html, 'release-1.22.1-ux.js?v=1.22.1', '1.22.1 UX runtime not wired.');
need(server, "const APP_VERSION = '1.22.1';", 'Server version not materialized.');
need(sw, 'flight-deck-efb-v1221-ux1', 'Service-worker cache not bumped for UX release.');
need(sw, "'/release-1.22.1.css?v=1.22.1'", '1.22.1 stylesheet not cached.');
need(sw, "'/release-1.22.1-ux.css?v=1.22.1'", '1.22.1 UX stylesheet not cached.');
need(sw, "'/release-1.22.1-ux.js?v=1.22.1'", '1.22.1 UX runtime not cached.');
...
need(uxCss, '.fd123-toast-stack', 'Non-blocking UX feedback styling missing.');
need(uxCss, '.fd123-context-actions', 'Context navigation styling missing.');
need(uxCss, '.fd123-resume-module', 'Resume-last-module styling missing.');
need(ux, 'flight-deck-last-module', 'Last-module persistence missing.');
need(ux, "qs('#app-home-button')", 'Reliable Home navigation missing.');
need(ux, 'function installContextActions()', 'Context quick navigation missing.');
need(ux, 'function installActionFeedback()', 'Action feedback missing.');
need(ux, "event.key !== 'Escape'", 'Escape-to-close behavior missing.');
need(ux, 'Wähle zuerst eine Taxi-Route.', 'Disabled-state explanation missing.');
need(ux, 'monitorOperationalFeedback', 'Operational error feedback missing.');
...
console.log('Flight Deck EFB 1.22.1 cockpit UI + pilot UX regression checks passed.');
