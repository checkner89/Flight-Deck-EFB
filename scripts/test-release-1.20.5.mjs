...
const compatibleVersions = ['1.20.5', '1.20.6', '1.20.7', '1.20.8', '1.20.9'];
if (!compatibleVersions.includes(pkg.version)) throw new Error(`Unexpected package version: ${pkg.version}`);
...
const validFileCaches = [
  'flight-deck-efb-v1205-files1',
...
if (!validFileCaches.some((cache) => serviceWorker.includes(cache))) {
  throw new Error('File browser offline cache is not valid for the active release chain.');
}
